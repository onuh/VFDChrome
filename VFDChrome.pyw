...
if __name__ == "__main__":
    ...
    if extract_version_folder() == None:
        error1 = Mbox('Chrome Error', 'Google Chrome Browser is not Installed on Your Computer. You need Google Chrome installed to use this application. Do you want to run Google Chrome Installer?', 0x10|0x4)
        if error1 == 6:

            subprocess.Popen(['ChromeSetup.exe'], shell=True, stdin=None, stdout=None, stderr=None, close_fds=True)
            # webbrowser.open('https://www.google.com/chrome/next-steps.html?statcb=1&installdataindex=empty&defaultbrowser=0#')
            sys.exit()
        elif error1 == 7:
            sys.exit()
